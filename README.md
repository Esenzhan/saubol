...
- Регистрация / вход (JWT), админ-панель со списком аккаунтов
- Загрузка документов (PDF/JPG/PNG), асинхронный OCR + через один AI-вызов:
  извлечение биомаркеров (включая качественные результаты вроде «не
  обнаружено», не только числа), придумывание понятного названия документа
  и определение папки — см. `analyzeDocument()` в `backend/services/ai.js`
- Оригинал файла хранится в самой БД (Postgres BYTEA) и открывается по
  кнопке «открыть оригинал» — не зависит от диска Render
- Страница «Документы»: дерево папок (Анализы и подпапки по типу анализа,
  Генетика, Выписки и заключения, Дневник наблюдений), сортировка по дате
- Медкарта: диагнозы/лекарства/рекомендации/аллергии + график динамики
  показателя с диапазоном нормы, единицей измерения и подсветкой
  подозрительных (возможно неверно распознанных) значений
- Защита от типичной ошибки OCR (потеря десятичного разделителя) —
  подозрительные значения помечаются `flagged_for_review`, а не тихо
  сохраняются как есть
- AI-чат, отвечающий с учётом ваших анализов и записей медкарты
- Кэширование данных между страницами (swr) — не перезагружает всё заново
  при каждом переходе

Подробности архитектурных решений — в `CLAUDE.md`.

## Что стоит доработать перед реальным использованием

1. **OCR точность** — tesseract.js слабее на бланках со сложной вёрсткой;
   помогает увеличение scale при рендере страницы (см. `ocr.js`), но для
   продакшена в идеале — облачный OCR (Google Vision / Yandex Vision / AWS
   Textract).
2. **Шифрование данных** — для медицинских данных в проде нужно шифрование
   на уровне хранилища/полей и, в идеале, юридическая консультация по 152-ФЗ.
3. **Мобильная версия / Apple Health** — потребует нативного слоя, веб не сможет.
