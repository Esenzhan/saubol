...
export async function analyzeDocument(rawText) {
  const response = await anthropic.messages.create({
    ...
    system:
      ...
      '"Общий анализ крови" БЕЗ даты (дата показывается на сайте отдельно, ' +
      "рядом со своим полем document_date — не дублируй её в названии). " +
      ...
      '"Комплексный анализ (гормоны, ОАК, IgE)". ' +
      "document_date — дата регистрации заявки/забора биоматериала из бланка (не дата печати бланка), " +
      "в формате YYYY-MM-DD. Если документ охватывает несколько дат (архив за период) — возьми самую позднюю.\n\n" +
